...
class PyTux:
    ...
    def __init__(self, screen_width=128, screen_height=96, num_agents=1):
        ...
        self.num_agents = num_agents  # New parameter for multi-agent support
        self.karts = []
    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None):
        if self.karts:
            ...
            if self.karts:
                for kart in self.karts:
                    kart.stop()
                del self.karts
            config = pystk.RaceConfig(num_kart=self.num_agents, laps=1, track=track)
            config.players[0].controller = pystk.PlayerConfig.Controller.PLAYER_CONTROL

            self.karts = [pystk.Race(config) for _ in range(self.num_agents)]
            for kart in self.karts:
                kart.start()
                kart.step()
        ...
        for t in range(max_frames):
            ...
            actions = []
            for i, kart in enumerate(state.players):
                kart = state.players[i]
                if np.isclose(kart.overall_distance / track.length, 1.0, atol=2e-3):
                    if verbose:
                        print(f"Agent {i} finished at t={t}")
                    break
                ...
                if data_callback is not None:
                    data_callback(t, np.array(self.karts[i].render_data[0].image), aim_point_image)

                if planner:
                    image = np.array(self.karts[i].render_data[0].image)
                ...
                if current_vel < 1.0 and t - last_rescue[i] > RESCUE_TIMEOUT:
                    last_rescue[i] = t
                    action.rescue = True

            if verbose:
                ax.clear()
                ax.imshow(self.karts[0].render_data[0].image)
            ...
            for kart, action in zip(self.karts, actions):
                kart.step(action)
            t += 1
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
    pytux = PyTux(num_agents=3)  # Example for 3 agents
